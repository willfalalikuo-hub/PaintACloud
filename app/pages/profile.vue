  ...
  <div class="p-4 md:p-8 max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-2xl p-6 border border-gray-200 mb-6">
      <div v-if="isLoggedIn" class="flex items-center gap-4">
        <div class="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center">
          <span class="text-xl font-bold text-gray-700">{{ user?.name?.charAt(0) || 'U' }}</span>
        </div>
        <div class="flex-1">
          <h2 class="font-bold text-gray-800 text-lg">{{ user?.name || '用户' }}</h2>
          <p class="text-gray-400 text-sm">{{ user?.email }}</p>
        </div>
        <button @click="handleLogout" class="text-sm text-gray-400 hover:text-gray-900 transition-colors">退出</button>
      </div>
      <div v-else class="flex items-center gap-4">
        ...
        <NuxtLink to="/auth/login" class="ml-auto bg-gray-900 text-white px-5 py-2 rounded-full text-sm font-medium hover:bg-gray-800 transition-colors">
          登录
        </NuxtLink>
      </div>
    </div>
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
      <div class="bg-white rounded-2xl p-4 border border-gray-200 text-center">
        <p class="text-2xl font-bold text-gray-900">{{ practices.length }}</p>
        <p class="text-xs text-gray-400 mt-1">作品</p>
      </div>
      <div class="bg-white rounded-2xl p-4 border border-gray-200 text-center">
        <p class="text-2xl font-bold text-gray-900">{{ enrollCount }}</p>
        <p class="text-xs text-gray-400 mt-1">课程</p>
      </div>
      <div class="bg-white rounded-2xl p-4 border border-gray-200 text-center">
        <p class="text-2xl font-bold text-gray-900">{{ streakDays }}</p>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 mb-6 overflow-hidden">
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
        ...
        <NuxtLink to="/canvas" class="text-sm text-gray-900 hover:underline">去画画</NuxtLink>
      </div>
      ...
      <div v-if="practices.length" class="grid grid-cols-2 sm:grid-cols-3 gap-3 p-4">
        <div v-for="p in practices" :key="p.id" class="bg-gray-50 rounded-xl overflow-hidden">
          <div class="aspect-square bg-gray-100 flex items-center justify-center relative">
            ...
            <!-- Score badge -->
            <div v-if="p.ai_score" class="absolute bottom-1 right-1 bg-white/90 rounded-full px-1.5 py-0.5 text-xs font-bold text-gray-900">
              {{ getScoreObj(p)?.total || '-' }}
            </div>
          </div>
          <div class="p-2">
      ...
      <div v-else class="text-center py-12 text-gray-300">
        ...
        <NuxtLink to="/" class="inline-block mt-2 text-sm text-gray-900 hover:underline">去选个课程开始吧</NuxtLink>
      </div>
    </div>

    <!-- Menu -->
    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden mb-20 md:mb-8">
      <NuxtLink v-for="item in menuItems" :key="item.label" :to="item.to"
        class="flex items-center px-6 py-4 border-b border-gray-100 hover:bg-gray-50 cursor-pointer transition-colors">
